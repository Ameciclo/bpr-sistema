...
bool requestConfigFromBase() {
    ...
    // TODO: Implementar comunicação BLE para solicitar config
    // Por enquanto, simula recebimento de config da base
    
    // Simulação: usa config padrão por enquanto
    Serial.println("⚠️ Simulation: Using default config (implement BLE config exchange)");
    
    // Salva a configuração recebida
    saveConfig();
