...
const { InteractiveEmulator } = require('./src/interactive');
...
async function main() {
  ...
  const { mode } = await inquirer.prompt([{
    ...
    choices: [
      { name: '🎮 Modo Interativo (recomendado)', value: 'interactive' },
      { name: '🎬 Cenários Automáticos', value: 'scenarios' }
    ]
  }]);

  if (mode === 'interactive') {
    const interactive = new InteractiveEmulator();
